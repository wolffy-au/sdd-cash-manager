...
This document captures the essential flows that define the `001-account-management` feature and links them to validation, persistence, and testing artifacts, while also surfacing how adjacent features (adjust balance, transactions, reconciliation) influence the core logic.
...
That service is wired through the API routers, which also trigger `lib.security_events` logging for sensitive operations (balances, merges) and coordinate with `AdjustmentService` for manual adjustments that originate from the same account lifecycle workflows described in `/specs/001-account-management`.
...
`AdjustmentService` coordinates the creation of `ManualBalanceAdjustment` and `AdjustmentTransaction` rows, while `ReconciliationService` reads those entries to produce `ReconciliationViewEntry` records that feed the reconciliation endpoint (see upcoming `/specs/005-reconciliation/`). Transactions also inform quick-fill suggestions (`QuickFillTemplate`) to keep the register fast and predictable.
...
These hierarchy checks also keep the reconciliation process honest, ensuring the balances across account groups feed into the difference reporting described in `specs/004-transaction-management` and the forthcoming reconciliation spec.
...
- Observability now includes structured logging from `lib/logging_config.py`, which reads `SDD_CASH_MANAGER_LOG_LEVEL` and is used across API handlers and database session scaffolding (`src/sdd_cash_manager/database.py`) to record lifecycle events.
 - `lib/security_events.py` emits audit-friendly entries for critical flows (balance adjustments, reconciliations, duplicate detections) and cooperates with the structured logger to scrub sensitive input so SonarCloud rules (pythonsecurity:S5145) remain satisfied.
...
- Running `pytest --cov=sdd_cash_manager --cov-report=term-missing` (already used in `scripts/pre_commit_checks.sh`) produces coverage reports in `build/coverage-reports` and ensures that the above flows remain covered. The current report (91.94%) is tracked as the baseline; the goal is to drive it toward 100% by adding guards/tests in the remaining Phase 6 tasks.
 - `tests/api` (e.g., `test_transactions.py`, `test_quickfill.py`, `test_reconciliation.py`) exercises the FastAPI contract end-to-end, including JWT auth, quick-fill payloads, reconciliation statuses, and validation error paths described in `specs/002-add-api-pytests`.
...
- Align the next `/specs/005-reconciliation/` deliverable with these flows so the reconciliation window, difference reporting, and status transitions plug directly into the services described above.
