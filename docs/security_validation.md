...
- `lib/validation.py` centralizes reusable sanitizers (name/description, amount normalization) so the same rules power the routers, services, and any new quick-fill or reconciliation inputs before they hit the database.
...
- `lib/security_events.py` captures critical operations (adjustments, reconciliations, duplicates) with contextual metadata, pairing with `lib/logging_config.py` so structured logs stay scrubbed and free of control characters or injection vectors (addressing pythonsecurity:S5145 style warnings recorded by SonarCloud).
...
- Logging hardening is reinforced through the `sanitize_for_logging()` helper in `lib/logging_config.py`, which strips control characters before text reaches the structured sink. This keeps audit logs safe from injection attempts while still providing rich context for SonarCloud/Snyk readers.
...
- Security-focused regression tests live in `tests/security/test_api_security.py`, which exercises JWT expiration, role enforcement, and forbidden configuration permutations. These tests run inside `scripts/pre_commit_checks.sh` alongside Ruff/Pyright/Mypy, ensuring the guarded flows stay covered before a commit reaches SonarCloud.
