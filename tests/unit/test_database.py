...
def patch_database_globals():
    ...
    yield mock_session_instance, mock_metadata, mock_session_factory
    ...
    Base.metadata = original_Base_metadata
...
def test_get_db_yields_session(patch_database_globals):
    ...
    mock_session_instance, _, mock_session_factory = patch_database_globals
...
def test_get_db_closes_session(patch_database_globals):
    ...
    """
    # Unpack the mocks provided by the fixture
    mock_session_instance, _, _ = patch_database_globals
...
def test_create_tables_calls_metadata_create_all(patch_database_globals):
    ...
    """
    # Unpack the mocks provided by the fixture
    _, mock_metadata, _ = patch_database_globals

    # Call the `create_tables` function. This function uses the patched global
    # `database.engine` and `Base.metadata`.
    database.create_tables()

    assert Base.metadata is mock_metadata

    # Assert that the `create_all` method on our mocked metadata
