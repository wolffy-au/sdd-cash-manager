...
```bash
uv run uvicorn sdd_cash_manager.main:app --reload --host 0.0.0.0 --port 8000
```
...
```bash
uv run pytest tests/integration/test_adjustment_api.py tests/integration/test_reconciliation_api.py \
...
- Use `uv run pytest --last-failed` during iteration to rerun only failing cases.
...
- To test the PostgreSQL-ready path, point `SDD_CASH_MANAGER_DATABASE_URL` at a Postgres instance and rerun the adjustment & reconciliation suites (`uv run pytest tests/integration/test_adjustment_reconciliation_flow.py`), ensuring Alembic migrations are up-to-date.
